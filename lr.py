...
def train(model, train_generator, validation_generator, train_n, val_n, epoch_n, model_save_path='results/cnn.h5',
          log_dir="results/logs/", best_path="results/best2.h5", batch_size=128, use_dynamic_lr=False):
    """
...
def main():
    ...
    history, model = train(model, train_generator, test_generator, len(img_list),
                           len(img_list), epoch_n, model_save_path, log_dir, best_path, batch_size, use_dynamic_lr=True)
